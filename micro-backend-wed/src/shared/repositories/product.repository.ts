...
export class ProductRepository {
  ...
  async findOneAndUpdate(query: any, data: any) {
    const product = await this.productModel.findOneAndUpdate(query, data, {
      new: true,
    });
    return product;
  }
  async deleteOneProduct(query: any) {

    const deletedProduct = await this.productModel.findOneAndDelete(query);
  ...
  async findProductWithGroupBy() {
    const product = await this.productModel.aggregate([
      {
        $facet: {
          lastestProducts: [{ $sort: { createAt: -1 } }, { $limit: 4 }],
          topRatingProducts: [{ $sort: { avgRating: -1 } }, { $limit: 8 }],
        },
      },
    ]);
    return product;
  ...
  async removeLicense(query: any) {
    const license = await this.licenseModel.findByIdAndDelete(query);
    return license;
  }
