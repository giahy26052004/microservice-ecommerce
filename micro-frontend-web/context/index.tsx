...
const cartReducer = (
  ...
  switch (action.type) {
    case "ADD_TO_CART":
      ...
      window.localStorage.setItem("_micro_cart", JSON.stringify(cartItems));
      return cartItems;
    case "REMOVE_FROM_CART":
      ...
      window.localStorage.setItem("_micro_cart", JSON.stringify(newCartItems));
      return newCartItems;
    case "UPDATE_CART":
    ...
    case "CLEAR_CART":
      ...
      window.localStorage.removeItem("_micro_cart");
      return [];
    default:
      return state;
  }
...
const Provider = ({ children }: Props) => {
  ...
  useEffect(() => {
    dispatch({
      type: "LOGIN",
      payload: JSON.parse(window.localStorage.getItem("_micro_user") || "{}"),
    });
    // get cart items from localStorage
    const cartItems = JSON.parse(
      window.localStorage.getItem("_micro_cart") || "[]"
    );
